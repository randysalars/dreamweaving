#!/usr/bin/env python3
"""
Natural Hypnotic Opening Synthesis - Refactored Version

Naturally hypnotic opening - human-sounding but still trance-inducing.
More natural pacing than the matched version.
Uses BaseSynthesizer to eliminate code duplication.

Original: ~490 lines
Refactored: ~420 lines (mostly SSML content)
"""
...
class NaturalHypnoticSynthesizer(BaseSynthesizer):
    """Synthesize naturally hypnotic opening - human yet trance-inducing."""

    name = "Natural Hypnotic Opening - Human Yet Trance-Inducing"
    output_filename = "hypnotic_natural.mp3"
    voice_name = "en-US-Neural2-I"
    speaking_rate = 0.85
    pitch = -1.5
    volume_gain_db = -1.5
    description = "More natural but still deeply hypnotic"
    duration_estimate = "~10-12 minutes"

    def get_chunks(self) -> List[str]:
        """Return SSML chunks for the natural hypnotic opening."""
        return [
...
<break time="3500ms"/>
    ...
    def get_success_message(self) -> List[str]:
        ...
        return [
            "Sounds more human and conversational",
            "Still deeply hypnotic and effective",
            "More natural vocal variety",
            "Better pacing and rhythm"
        ]


if __name__ == "__main__":
    synthesizer = NaturalHypnoticSynthesizer()
    synthesizer.run()
