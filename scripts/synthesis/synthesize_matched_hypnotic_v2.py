#!/usr/bin/env python3
"""
Matched Hypnotic Opening Synthesis - Refactored Version

Hypnotic opening matched to reference voice with very slow pace,
60% pause ratio, and soft intimate delivery.
...
class MatchedHypnoticSynthesizer(BaseSynthesizer):
    """Synthesize hypnotic opening matched to reference voice."""

    name = "Hypnotic Opening - MATCHED TO REFERENCE VOICE"
    output_filename = "hypnotic_opening_matched.mp3"
    voice_name = "en-US-Neural2-I"
    speaking_rate = 0.72
    pitch = -3.0
    volume_gain_db = -3.0
    description = "Very slow pace (60% silence), soft intimate delivery"
    duration_estimate = "~12-15 minutes"

    def get_chunks(self) -> List[str]:
        """Return SSML chunks for the matched hypnotic opening."""
        return [
...
<break time="6000ms"/>
    ...
    def get_success_message(self) -> List[str]:
        ...
        return [
            "Very slow pace (60% pauses)",
            "Soft, intimate volume",
            "Deep hypnotic delivery",
            "Extended silence for trance depth"
        ]


if __name__ == "__main__":
    synthesizer = MatchedHypnoticSynthesizer()
    synthesizer.run()
