#!/usr/bin/env python3
"""
Hypnotic Closing Synthesis - Refactored Version

This version uses BaseSynthesizer to eliminate code duplication.

Original: ~290 lines
Refactored: ~200 lines (mostly SSML content)
"""
...
class ClosingSynthesizer(BaseSynthesizer):
    """Synthesize hypnotic closing/emergence - gradual return to awareness."""

    name = "Hypnotic Closing - Gradual Emergence"
    output_filename = "hypnotic_closing.mp3"
    voice_name = "en-US-Neural2-I"
    speaking_rate = 0.88
    pitch = -1.0
    volume_gain_db = -1.0
    description = "Progressive awakening with post-hypnotic suggestions"
    duration_estimate = "~8-10 minutes"

    def get_chunks(self) -> List[str]:
        """Return SSML chunks for the closing."""
        return [
...
<break time="1500ms"/>
    ...
    def get_success_message(self) -> List[str]:
        ...
        return [
            "Integrates insights and wisdom",
            "Creates mental anchor for future use",
            "Gradually returns to full awareness",
            "Installs post-hypnotic suggestions",
            "Ends with clear, energized state",
            "Includes channel call-to-action"
        ]


if __name__ == "__main__":
    synthesizer = ClosingSynthesizer()
    synthesizer.run()
