#!/usr/bin/env python3
"""
Hypnotic Opening Synthesis - Refactored Version

Deep relaxation induction with progressive muscle relaxation,
guided visualization, and 10-to-1 countdown deepener.
Uses BaseSynthesizer to eliminate code duplication.

Original: ~475 lines
Refactored: ~400 lines (mostly SSML content)
"""
...
class HypnoticOpeningSynthesizer(BaseSynthesizer):
    """Synthesize hypnotic opening - deep relaxation induction."""

    name = "Hypnotic Opening - Deep Relaxation Induction"
    output_filename = "hypnotic_opening.mp3"
    voice_name = "en-US-Neural2-I"
    speaking_rate = 0.75
    pitch = -3.0
    volume_gain_db = -2.0
    description = "Deep hypnotic male voice - slow, soothing"
    duration_estimate = "~10-12 minutes"

    def get_chunks(self) -> List[str]:
        """Return SSML chunks for the hypnotic opening."""
        return [
...
<break time="5s"/>
    ...
    def get_success_message(self) -> List[str]:
        ...
        return [
            ...
            "10-to-1 countdown deepener",
            "Sanctuary establishment"
        ]


if __name__ == "__main__":
    synthesizer = HypnoticOpeningSynthesizer()
    synthesizer.run()
