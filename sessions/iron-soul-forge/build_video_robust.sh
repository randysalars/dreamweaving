...
echo "Audio duration: $DURATION seconds (30 minutes)"
...
NUM_IMAGES=${#IMAGES[@]}
IMAGE_DURATION=163  # seconds per image

echo "Creating video segments for $NUM_IMAGES images..."

# Step 1: Create individual video segments from each image
for i in "${!IMAGES[@]}"; do
    IMG="$IMG_DIR/${IMAGES[$i]}"
    SEG="$TEMP_DIR/segment_$(printf "%02d" $i).mp4"
    echo "Creating segment $i: ${IMAGES[$i]}"
    
    ffmpeg -y -loop 1 -i "$IMG" -t $IMAGE_DURATION \
...
# Step 2: Create concat file for segments
...
ffmpeg -y -f concat -safe 0 -i "$CONCAT_FILE" \
    -c copy \
    "$TEMP_DIR/video_only.mp4" 2>/dev/null

echo "Adding audio track..."

# Step 4: Add audio
ffmpeg -y -i "$TEMP_DIR/video_only.mp4" -i "$AUDIO" \
    -c:v copy -c:a aac -b:a 192k \
    -shortest \
    "$OUTPUT_DIR/iron-soul-forge-final.mp4" 2>/dev/null
...
echo ""
echo "=== VIDEO CREATED ==="
ls -lh "$OUTPUT_DIR/iron-soul-forge-final.mp4"
echo ""
echo "Verifying output..."
ffprobe -v error -show_entries format=duration,size -of default=noprint_wrappers=1 "$OUTPUT_DIR/iron-soul-forge-final.mp4"
