...
type ContentEventBody = {
  unlock_token?: string;
  event?: "play_start" | "play_end" | "stream_start" | "stream_end" | "download" | "access";
  duration_ms?: number;
  success?: boolean;
...
function normalizeEventName(e: string): string | null {
  switch (e) {
    case "access":
      return "content_access";
    case "play_start":
      return "content_play_start";
    case "play_end":
      return "content_play_end";
    case "stream_start":
      return "content_stream_start";
    case "stream_end":
      return "content_stream_end";
    case "download":
...
export async function POST(request: NextRequest) {
  ...
  const data = body as ContentEventBody;
  ...
  const eventKey = typeof data.event === "string" ? data.event : "access";
  const name = normalizeEventName(eventKey);
  if (!name) return NextResponse.json({ ok: false, error: "invalid_event" }, { status: 400 });

  const sessionId = data.session_id || request.cookies.get(SESSION_COOKIE)?.value || null;
  const sql = getSql();

  const rows = (await sql`
    select f.order_id::text as order_id, f.product_sku, f.revoked_at::text as revoked_at, f.revoke_reason, o.user_id
  ...
  const durationMs = typeof data.duration_ms === "number" && Number.isFinite(data.duration_ms) ? Math.max(0, data.duration_ms) : null;
  ...
  return NextResponse.json({ ok: true, order_id: row.order_id, name });
}
